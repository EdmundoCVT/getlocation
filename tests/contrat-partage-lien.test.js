// tests/contrat-partage-lien.test.js
//
...
//
// Exécute le vrai code de contrat.html (smsHref()) via jsdom, comme
...
test("smsHref : format iOS (\"&body=\", sans \"?\") sur iPhone — c'est le bug signalé, corrigé ici", () => {
  ...
  const href = win.smsHref("bonjour%20test");
  assert.equal(href, "sms:&body=bonjour%20test");
});
...
test("smsHref : format standard (\"?body=\") sur desktop (fallback par défaut)", () => {
