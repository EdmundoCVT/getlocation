...
### 0.6 Dossier contrat sécurisé (`contrat.html`, remise/restitution du véhicule, signature)

`contrat.html` combinait jusqu'ici deux modes purement client (`?data=`/`?prefill=`, base64, aucun lien avec la réservation ni avec le serveur) et une soumission de signature vers un formulaire Netlify Forms — mort depuis la Phase B (site et fonctions serveur unifiés sur Cloudflare, voir §0). Ces deux liens **restent inchangés et fonctionnels** (compatibilité des liens déjà envoyés par e-mail avant cet ajout), mais un nouveau lien sécurisé, **basé sur un jeton et associé à la réservation**, est désormais utilisé pour toute nouvelle réservation :

- `src/lib/contract-dossier-token.js` : même schéma HMAC-SHA-256 que `document-access-token.js`/`agency-document-token.js` (réutilise le secret `DOCUMENT_TOKEN_PEPPER` déjà configuré, jamais un nouveau secret), avec **deux espaces de jetons distincts** — un jeton **agence** (lecture/écriture du dossier, émis automatiquement au paiement comme le jeton documentaire, valable jusqu'à 30 jours après la restitution, plancher 60 jours) et un jeton **client** (lecture + signature uniquement, émis à la demande par l'agence via l'action "envoyer au client", valable 30 jours).
- Les deux jetons voyagent en **fragment d'URL** (`contrat.html#agencyToken=...` / `#clientToken=...`), jamais en paramètre de requête — un fragment n'est jamais transmis au serveur par le navigateur (pas dans les journaux d'accès, pas dans un en-tête Referer), contrairement à un `?agencyToken=`.
- `src/api/contract-dossier-agency.js` (`GET`/`POST /api/contract-dossier-agency`) : lecture du dossier + actions `update-fields` (adresse, permis, second conducteur, immatriculation, mode de règlement caution — jamais le véhicule/les dates/le prix, qui restent lus depuis la réservation elle-même), `send-to-client` (bloqué avec la liste précise des champs manquants tant que l'adresse complète, le numéro de permis et — si sélectionné — les informations du second conducteur ne sont pas tous renseignés), `update-depart`/`update-retour` (état des lieux remise/restitution : date/heure, kilométrage, carburant, propreté, dommages, référence des photos — pas de téléversement, uniquement un champ texte —, clés/accessoires, agent, signatures client/agence saisies sous forme de nom tapé), `update-observations`.
- `src/api/contract-dossier-client.js` (`GET`/`POST /api/contract-dossier-client`) : lecture du récapitulatif (jamais d'écriture sur le dossier remise/restitution) + signature (case CGL + image du pad de signature, avec le même contrôle de version des CGL — `cglVersion`/`CGL_VERSION` — que `validate-reservation-input.js`).
...
**Statut au 16/09/2026 : backend terminé et testé (unitairement), pas encore relié à l'interface `/contrat`.** Objectif : remplacer, à terme, la caution "à la main" (`src/lib/deposits.js`, espèces/virement/carte physique) par une vraie empreinte bancaire quand le client paie par carte à distance — le montant est **bloqué** chez le client, jamais débité automatiquement.

- **Table dédiée** : `migrations/0005_deposit_authorizations.sql` (à appliquer comme les migrations précédentes) — **jamais** dans la table `deposits` existante, qui reste le mécanisme "caution classique" inchangé.
- **Secret dédié `MOLLIE_DEPOSIT_API_KEY`** (voir §0.2), **strictement séparé** de `MOLLIE_API_KEY` (paiement de location, déjà en mode live en production) : une clé de test ici ne peut jamais interférer avec un vrai paiement de location, et inversement. Configurer avec `npx wrangler secret put MOLLIE_DEPOSIT_API_KEY`, avec un jeton Mollie **`test_...`** (jamais `live_...` tant que cette fonctionnalité n'a pas été validée en conditions réelles).
- **Logique métier** : `src/lib/deposit-authorizations.js` — crée le paiement Mollie (`method: "creditcard"`, `captureMode: "manual"`), traduit les statuts Mollie réels (`open`/`pending`/`authorized`/`paid`/`canceled`/`expired`/`failed`) en statuts internes GET LOCATION, capture tout ou partie du montant (validé serveur contre le solde restant), libère l'autorisation (annulation Mollie officielle, jamais une simple mise à jour locale).
- **Endpoints** : `/api/agency-deposit-authorizations` (agence, session requise — créer/débiter/libérer/rafraîchir) et `/api/mollie-deposit-webhook` (public, revérifie toujours le statut réel auprès de Mollie avant d'agir, comme `/api/mollie-webhook`).
- **Page de retour** : `caution-mollie-retour.html` — page statique minimale (bilingue FR/EN, sans dépendance JS) vers laquelle Mollie redirige le client après saisie de sa carte. `/contrat` ne propose pas encore de bouton pour générer/partager ce lien (prochaine étape, une fois le test réel ci-dessous validé).
- **Test réel de bout en bout** : `MOLLIE_DEPOSIT_API_KEY=test_xxx node scripts/test-deposit-authorization.js` — crée une vraie préautorisation Mollie test de 500 €, affiche le lien de paiement, attend le résultat et affiche la réponse Mollie complète. **N'a pas pu être exécuté dans l'environnement utilisé pour préparer cette intégration** (aucune clé disponible **et** accès réseau sortant vers `api.mollie.com` bloqué par la sandbox, comme documenté en §0.3 pour le paiement de location) — à lancer manuellement dès que la clé est configurée, depuis un poste avec accès réseau normal.
- **Point à vérifier lors de ce premier test réel** : le nom exact du champ Mollie indiquant la date limite de capture d'une autorisation carte (`payment.authorizationExpiresAt` dans le code actuel, jamais vérifié en conditions réelles ici) — voir le commentaire dédié dans `src/lib/deposit-authorizations.js#syncFromMolliePayment`. Si Mollie renvoie un autre nom de champ, ce script l'affichera dans la réponse JSON complète ; ajuster ce fichier en conséquence avant de relier l'interface `/contrat`.
- **Prochaine étape** (non commencée) : bloc "DÉPÔT DE GARANTIE" dans `contrat.html` (créer l'empreinte, copier/ouvrir le lien, afficher le statut, actions "Libérer"/"Débiter"), avec un bandeau "MOLLIE — MODE TEST" bien visible tant que `MOLLIE_DEPOSIT_API_KEY` est un jeton `test_...` (déjà renvoyé par l'API sous `mollieTestMode` dans `GET /api/agency-deposit-authorizations`).
