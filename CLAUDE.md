...
- Empreinte bancaire Mollie pour le dépôt de garantie (backend + tests faits, voir `DEPLOIEMENT.md` §0.7) : reste à (1) faire tourner `scripts/test-deposit-authorization.js` en conditions réelles dès que `MOLLIE_DEPOSIT_API_KEY` est configurée, pour confirmer le nom exact du champ Mollie de date limite de capture, puis (2) relier l'interface agence `/contrat` (bloc "DÉPÔT DE GARANTIE" décrit dans la demande d'origine). Toujours en clé `test_...` — ne jamais passer `MOLLIE_DEPOSIT_API_KEY` en `live_...` avant validation complète.
